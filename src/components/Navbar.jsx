                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const response = await API.post('/orders', { items: payloadItems });
                        const order = response.data;
                        
                        const message = `Olá Genjoy, acabei de enviar o pedido #${order.order_number} no site! Aguardo instruções de pagamento.`;
                    ...
                    className="w-full py-3 bg-[#25D366] hover:bg-[#20ba5a] text-white font-bold text-sm rounded-xl flex items-center justify-center gap-2 transition-all shadow-md cursor-pointer border-none disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {checkoutLoading ? (
                    ...
                    {checkoutLoading ? 'Processando Pedido...' : 'Concluir Pedido via WhatsApp'}
                  </button>
                </div>
              )}
            </div>
